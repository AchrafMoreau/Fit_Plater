...
const TABLE_HEAD = ["ID", "Productivity"];

export function Productivities() {
  ...
  const handelEdite = (id) =>{
    ...
    setEdiOrAdd("Edite")
    setToggel(true);
    axios.get(`${import.meta.env.VITE_APP_BACKEND_HOST}/api/prod/${id}`, config)
      .then(({data}) => setName(data.productivity))
  ...
  const handleSubmit = (e)=>{
    e.preventDefault();
    axios.put(`${import.meta.env.VITE_APP_BACKEND_HOST}/api/prod/${ID}`, {productivity:name}, config)
      .then(({data})=> {
        // setToggel(false);
  ...
  const handelDelete = ()=>{
    axios.delete(`${import.meta.env.VITE_APP_BACKEND_HOST}/api/prod/${ID}`, config)
  ...
  const handelAddSubmit = ()=>{
    axios.post(`${import.meta.env.VITE_APP_BACKEND_HOST}/api/prod`,{productivity: name}, config)
      .then(() => window.location.reload())
      .catch((err) => console.log(err))
  }
  const handelAdd = ()=>{
  ...
  useEffect(() => {
    ...
    .then(({ data }) => setProductivities(data.productivity))
    .catch((err) => console.log(err));
  }, []);

  return (
    <>
    {notification && <Notification content={notifContent} />}
    <Card className="h-full w-full p-4">
        ...
        <div className="mb-4 flex flex-col justify-between gap-8 md:flex-row md:items-center">
          <div>
            ...
            <Typography color="gray" className="mt-1 font-normal">
                Here are the details for all productivities
            </Typography>
          </div>
          <div className="flex w-full shrink-0 gap-2 md:w-max">
            ...
            />
              ...
              <Button onClick={handelAdd} className="flex items-center gap-3 text-gray-900" size="sm">
                <PlusIcon strokeWidth={2} className="h-4 w-4" /> Add Productivity
              </Button>
          </div>
        </div>
      </CardHeader>
      <CardBody className="overflow-scroll px-0">
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {productivities.map(({ id, productivity }) => (
              <tr key={id}>
                <td className="p-4 border-b border-blue-gray-50">{id}</td>
                <td className="p-4 border-b border-blue-gray-50">{productivity}</td>
